...
class VarQRTE(VarQTE):
    ...
    Algorithms that use McLachlans variational principle to compute the real time evolution
    for a given Hermitian operator (Hamiltonian) and quantum state.

    ∑ _j Re(A_i,j) dθ_j/dt = Im(C_i)
    """
    ...
    def _prepare_op(self, operator: OperatorBase) -> OperatorBase:
        ...
        """
        # The NaturalGradient implementation in qiskit/aqua/operators/gradients evaluates
        # ∑ _j Re(A_i,j) dθ_j/dt = -Re(C_i)
        # using -Re(iz) = -Im(z)enables to compute
        # ∑ _j Re(A_i,j) dθ_j/dt = -Re(-i C_i) = Im(C_i)
        return -1j * operator
